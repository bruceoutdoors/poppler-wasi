...
- **Sandbox untrusted PDFs.** Run with only the capabilities and resources
  you grant it; limits the impact of a parser
...
```
A `.cwasm` is native code tied to the exact Wasmtime version that produced it. Further reading:
...
Poppler requires some patches to be built with WASI - see [patches/poppler.patch](patches/poppler.patch).
...
The Poppler version is pinned by submodule referencing the canonical poppler git repo.
...
modifications, [patches/poppler.patch](patches/poppler.patch).
